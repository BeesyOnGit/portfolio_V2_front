...
import { SkeletonTerminal } from "./skeletons/SkeletonTerminal";
...
const Help: React.FC = () => (
    <div>
        ...
        <ul className="space-y-1">
            <li>
                <span className="text-dark-yellow w-32 inline-block">help</span> Show this help message.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">whoami</span> Display my bio.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">home</span> Navigate to home.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">experience</span> Show my work experience.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">projects</span> List all projects.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">project &lt;id&gt;</span> Show project details.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">contact</span> Display contact information.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">theme &lt;light|dark&gt;</span> Switch color theme.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">mode classic</span> Switch to classic UI.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">login</span> Access the admin dashboard.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">clear</span> Clear the terminal screen.
            </li>
            <li>
                <span className="text-dark-yellow w-32 inline-block">date</span> Show the current date.
            </li>
        </ul>
    </div>
);
                    ...
                    {experienceData.map((exp: any, i: number) => (
                        <div key={i} className="mb-4">
                            <p className="font-bold text-dark-yellow">
                                {exp.role} <span className="text-dark-cyan">@ {exp.company}</span>
                            </p>
                            <p className="text-dark-comment">
                ...
                <div>
                    ...
                    {projectsData.map((p: any, i: number) => (
                        <div key={i} className="flex gap-1">
                            ...
                            {p.tech.length > 0 && (
                                <div className="flex text-dark-blue pl-2">
                                    (
                                    {p.tech?.map((tech: any, i) => (
                                        <span className="text-dark-blue">{tech.name + (i < p.tech.length - 1 ? " | " : "")}</span>
                                    ))}
                                    )
                                </div>
                            )}
                ...
                <div>
                    {siteData.socials.map((s: any, i: number) => (
                        <div key={i} className="flex items-center gap-4">
                            ...
                            <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-dark-cyan hover:underline">
                                {s.url}
                            </a>
                        </div>
                    ))}
...
const Terminal: React.FC = () => {
    const { setTheme, siteData, experienceData, projectsData, isLoading } = useAppContext();
    ...
    const [output, setOutput] = useState<React.ReactNode[]>([
        <div key="welcome">
            <p>
                Welcome to my portfolio! Type <span className="text-dark-yellow">'help'</span> to see a list of commands.
            </p>
        </div>,
    ]);
    ...
    const PROMPT = PROMPT_BASE.replace("{user}", siteData?.name?.toLowerCase().replace(/\s/g, "-") || "guest");
                ...
                case "project":
                    ...
                    if (project) {
                        newOutput = (
                            <div>
                                <p className="font-bold text-dark-yellow">
                                    {project.name} <span className="text-dark-comment">({project.year})</span>
                                </p>
                                <p className="mt-1">{project.description}</p>
                                <p className="mt-2 text-dark-cyan">{project.tech.map((t) => t.name).join(", ")}</p>
                            </div>
                        );
                    } else {
    ...
    const handleContainerClick = (e: React.MouseEvent) => {
        // Only focus input if clicking outside the input element itself
    ...
    if (isLoading) {
        return (
            <div className="font-mono p-4 h-screen w-full bg-dark-bg text-dark-fg text-base">
                <SkeletonTerminal />
            </div>
        );
    }
    ...
    return (
        <div className="font-mono p-4 h-screen w-full bg-dark-bg text-dark-fg text-base" onClick={handleContainerClick}>
